...
void Omnicar::Drive(void) {
	...
	if(command.dir == FL_DIAG_DIR) {
		MoveFB(command.left_right_ratio, 3);
	} else
	if(command.dir == FR_DIAG_DIR) {
		MoveFB(command.left_right_ratio, 2);
	} else
	if(command.dir == BL_DIAG_DIR) {
		MoveFB(command.left_right_ratio, 2);
	} else
	if(command.dir == BR_DIAG_DIR) {
		MoveFB(command.left_right_ratio, 3);
	} 
	/* 前後左右から±15度 */
	else
	if(command.dir == FL_15DEG_DIR) {
		MoveFB(command.front_back_ratio, 1);
	} else
	if(command.dir == LF_15DEG_DIR) {
		MoveFB(command.left_right_ratio, 3);
	} else
	if(command.dir == FR_15DEG_DIR) {
		MoveFB(command.front_back_ratio, 1);
	} else
	if(command.dir == RF_15DEG_DIR) {
		MoveFB(command.left_right_ratio, 2);
	} else
	if(command.dir == BL_15DEG_DIR) {
		MoveFB(command.front_back_ratio, 1);
	} else
	if(command.dir == LB_15DEG_DIR) {
		MoveFB(command.left_right_ratio, 2);
	} else
	if(command.dir == BR_15DEG_DIR) {
		MoveFB(command.front_back_ratio, 1);
	} else
	if(command.dir == RB_15DEG_DIR) {
		MoveFB(command.left_right_ratio, 3);
	}
	/* 停止 */
	else {
...
void Omnicar::MoveFB(char speed_ratio, char front_wheel) {
	...
	if(front_wheel == 1) {
		...
		} else if(command.dir == FRONT_DIR || command.dir == R_FRONT_DIR || command.dir == L_FRONT_DIR) {
			Motor1.Stop();
			Motor2.RotSettedSpeed(speed_ratio, 1);
			Motor3.RotSettedSpeed(speed_ratio, -1);
		} else if(command.dir == FL_15DEG_DIR) {
			Motor1.Stop();
			Motor2.RotSettedSpeed(speed_ratio * 0.95, 1);
			Motor3.RotSettedSpeed(speed_ratio, -1);
		} else if(command.dir == FR_15DEG_DIR) {
			Motor1.Stop();
			Motor2.RotSettedSpeed(speed_ratio, 1);
			Motor3.RotSettedSpeed(speed_ratio * 0.95, -1);
		} else if(command.dir == BL_15DEG_DIR) {
			Motor1.Stop();
			Motor2.RotSettedSpeed(speed_ratio * 0.95, -1);
			Motor3.RotSettedSpeed(speed_ratio, 1);
		} else if(command.dir == BR_15DEG_DIR) {
			Motor1.Stop();
			Motor2.RotSettedSpeed(speed_ratio, -1);
			Motor3.RotSettedSpeed(speed_ratio * 0.95, 1);
		}
	...
	if(front_wheel == 2) {
		if(command.dir == BACK_DIR || command.dir == R_BACK_DIR || command.dir == L_BACK_DIR) {
			...
			Motor2.Stop();
			Motor3.RotSettedSpeed(speed_ratio, 1);
		} else if(command.dir == FR_DIAG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio * 0.95, -1);
			Motor2.Stop();
			Motor3.RotSettedSpeed(speed_ratio, -1);
		} else if(command.dir == BL_DIAG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio, 1);
			Motor2.Stop();
			Motor3.RotSettedSpeed(speed_ratio * 0.95, 1);
		} else if(command.dir == RF_15DEG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio * 0.95, -1);
			Motor2.Stop();
			Motor3.RotSettedSpeed(speed_ratio, -1);
		} else if(command.dir == LB_15DEG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio * 0.95, 1);
			Motor2.Stop();
			Motor3.RotSettedSpeed(speed_ratio, 1);
		}
	...
	/* 3番ホイールが正面 */
	{
		if(command.dir == BACK_DIR || command.dir == R_BACK_DIR || command.dir == L_BACK_DIR) {
			...
			Motor2.RotSettedSpeed(speed_ratio, -1);
			Motor3.Stop();
		} else if(command.dir == FL_DIAG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio * 0.95, 1);
			Motor2.RotSettedSpeed(speed_ratio, 1);
			Motor3.Stop();
		} else if(command.dir == BR_DIAG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio, 1);
			Motor2.RotSettedSpeed(speed_ratio * 0.95, 1);
			Motor3.Stop();
		} else if(command.dir == LF_15DEG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio * 0.95, 1);
			Motor2.RotSettedSpeed(speed_ratio, 1);
			Motor3.Stop();
		} else if(command.dir == RB_15DEG_DIR) {
			Motor1.RotSettedSpeed(speed_ratio * 0.95, -1);
			Motor2.RotSettedSpeed(speed_ratio, -1);
			Motor3.Stop();
		}
