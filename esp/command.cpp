...
void Command::generate_radial_command(void) {
    ...
    /* 前後方向と左右方向の比率から進行方向を求める */
        ...
        /* 左右方向成分が大きい場合 */
        else if(command.front_back_ratio < command.left_right_ratio){
        ...
        /* 前後方向の方が移動成分が大きい場合2 */
            ...
            if(command.fb_dir == FRONT_DIR) {
                 /* 前右方向 */
                if(command.lr_dir == RIGHT_DIR) {
                    command.dir = FR_15DEG_DIR;
                } 
                /* 前左方向 */
                else {
                    command.dir = FL_15DEG_DIR;
                }
            } else {
                 /* 後右方向 */
                if(command.lr_dir == RIGHT_DIR) {
                    command.dir = BR_15DEG_DIR;
                } 
                /* 後左方向 */
                else {
                    command.dir = BL_15DEG_DIR;
                }
            }
        }
        /* 左右方向の方が移動成分が大きい場合1 */
        ...
        /* 左右方向の方が移動成分が大きい場合2 */
            ...
            if(command.fb_dir == FRONT_DIR) {
                 /* 前右方向 */
                if(command.lr_dir == RIGHT_DIR) {
                    command.dir = RF_15DEG_DIR;
                } 
                /* 前左方向 */
                else {
                    command.dir = LF_15DEG_DIR;
                }
            } else {
                 /* 後右方向 */
                if(command.lr_dir == RIGHT_DIR) {
                    command.dir = RB_15DEG_DIR;
                } 
                /* 後左方向 */
                else {
                    command.dir = LB_15DEG_DIR;
                }
            }
        } 
        /* 前後方向と左右方向の移動成分が等しい場合 */
        else if(comp == 9) {
            if(command.fb_dir == FRONT_DIR) {
                if(command.lr_dir == LEFT_DIR) {
                    command.dir = FL_DIAG_DIR;
                } else {
                    command.dir = FR_DIAG_DIR;
                }
            } else {
                if(command.lr_dir == LEFT_DIR) {
                    command.dir = BL_DIAG_DIR;
                } else {
                    command.dir = BR_DIAG_DIR;
                }
            }
        } 
        /* 基本なし */
